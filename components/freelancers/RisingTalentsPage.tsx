// components/freelancers/RisingTalentsPage.tsx
'use client'
...
import { Star, MapPin, DollarSign, CheckCircle2, Eye, MessageCircle, Zap } from "lucide-react"

interface RisingTalentsPageProps {
  dict: any
  lang: string
}

export function RisingTalentsPage({ dict, lang }: RisingTalentsPageProps) {
  ...
  const fetchRisingTalents = async () => {
    try {
      const response = await fetch('/api/users/freelancers?sortBy=recent&minProjects=1&limit=12')
      const data = await response.json()
      if (response.ok) {
        setFreelancers(data.freelancers || [])
      }
    } catch (error) {
      console.error('Error fetching rising talents:', error)
    } finally {
      setLoading(false)
    }
  }
    ...
    {freelancer.skills.length > 3 && (
      ...
      {freelancers.length === 0 && (
        <div className="text-center py-12">
          <p className="text-slate-600 dark:text-slate-400">Aucun talent émergent pour le moment.</p>
        </div>
      )}
    </div>
  )
