...
import { 
  ...
  Award, Code2, FileText, HelpCircle, ExternalLink,
...
export function Navigation() {
  ...
  const navItems: NavItem[] = [
    ...
    { href: `/${lang}/talents`,    labelKey: 'navigation.talents',    icon: <Users className="h-4 w-4" />,       descriptionKey: 'navigation.talentsDesc' },
    { href: `/${lang}/gigs`,       labelKey: 'navigation.gigs',       icon: <Zap className="h-4 w-4" />,         descriptionKey: 'navigation.gigsDesc' },
    { href: `/${lang}/projects`,   labelKey: 'navigation.projects',   icon: <Rocket className="h-4 w-4" />,      descriptionKey: 'navigation.projectsDesc' },
    { href: `/${lang}/ai-matching`,labelKey: 'navigation.aiMatching', icon: <Sparkles className="h-4 w-4" />,   descriptionKey: 'navigation.aiMatchingDesc', badge: "AI" },
    {
  ...
  const externalLinks = [
    { labelKey: 'external.upwork', href: 'https://www.upwork.com',  icon: <ExternalLink className="h-4 w-4" />, description: 'Trouvez des freelances sur Upwork' },
    { labelKey: 'external.fiverr', href: 'https://www.fiverr.com',  icon: <ExternalLink className="h-4 w-4" />, description: 'Services freelance sur Fiverr' },
        ...
        <div className="flex h-16 items-center justify-between gap-2">
          ...
          <Link href={`/${lang}`} className="flex items-center gap-2 group flex-shrink-0">
            <div className="relative">
              <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs shadow-lg">
                NRB
              </div>
            ...
            {/* Hide full wordmark on very small screens */}
            <div className="hidden xs:flex flex-col">
              ...
              <span className="text-[10px] text-muted-foreground -mt-0.5 hidden sm:block leading-none">
          ...
          <div className="hidden lg:flex items-center gap-0.5 flex-1 justify-center">
            {navItems.map((item) => {
                ...
                return (
                  <DropdownMenu key={item.labelKey}>
                    <DropdownMenuTrigger asChild>
                      <button className={cn(
                        "group px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-1.5",
                    ...
                    <DropdownMenuContent align="center" className="w-60">
                      {item.children.map((child) => (
                        <DropdownMenuItem key={child.labelKey} asChild>
                          <Link href={child.href} className="flex items-center gap-3 cursor-pointer py-2">
                            <div className="p-1.5 rounded-lg bg-accent/30 flex-shrink-0">{child.icon}</div>
                            <div className="flex flex-col min-w-0">
                              <span className="font-medium text-sm">{t(child.labelKey)}</span>
                              <span className="text-xs text-muted-foreground truncate">{t(child.descriptionKey)}</span>
                            </div>
                          </Link>
                        </DropdownMenuItem>
                      ))}
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={cn(
                    "group relative px-3 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-1.5",
                    isActive(item.href)
                  ...
                  <span>{t(item.labelKey)}</span>
                  {item.badge && (
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-gradient-to-r from-blue-500 to-purple-500 text-white font-bold">
                      {item.badge}
                    </span>
                  )}
                </Link>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="gap-1 text-muted-foreground px-3">
                  <Globe className="h-4 w-4" />
                  <span className="text-sm hidden xl:inline">{t('external.platforms')}</span>
                ...
                {externalLinks.map((link) => (
                  <DropdownMenuItem key={link.href} asChild>
                    <a href={link.href} target="_blank" rel="noopener noreferrer"
                      ...
                      <div className="p-1.5 rounded-lg bg-accent/30">{link.icon}</div>
              ...
              <DropdownMenuContent align="end">
                {languages.map((l) => (
                  <DropdownMenuItem key={l.code} onClick={() => switchLanguage(l.code)}
                    ...
                    <span>{l.flag}</span>
                    <span>{l.label}</span>
                    {lang === l.code && <CheckCircle2 className="h-3.5 w-3.5 ml-auto text-green-500" />}
                  </DropdownMenuItem>
                ))}
              </DropdownMenuContent>
            </DropdownMenu>
          ...
          {/* ── Tablet nav (md–lg): show key links inline ─────────── */}
              ...
              <SheetContent side="right" className="w-full max-w-sm p-0 flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto">
                  <div className="px-4 py-4 space-y-5">
                        ...
                        if (item.children?.length) {
                          return (
                            <AccordionItem key={item.labelKey} value={item.labelKey}
                              ...
                              <AccordionTrigger className="px-4 py-3 text-sm font-semibold hover:no-underline hover:bg-accent/30 transition-colors">
                                <div className="flex items-center gap-3">
                                  <div className="p-1.5 rounded-lg bg-accent/40">{item.icon}</div>
                                  <span>{t(item.labelKey)}</span>
                                </div>
                              </AccordionTrigger>
                              <AccordionContent className="pb-0">
                                <div className="px-2 pb-2 space-y-1">
                                  {item.children.map((child) => (
                                    <Link key={child.labelKey} href={child.href}
                                      onClick={() => setMobileMenuOpen(false)}
                                      className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-all hover:bg-accent/50">
                                      <div className="p-1.5 rounded-lg bg-accent/30 flex-shrink-0">{child.icon}</div>
                                      <div className="flex-1 min-w-0">
                                        <p className="font-medium leading-none mb-0.5">{t(child.labelKey)}</p>
                                        <p className="text-xs text-muted-foreground truncate">{t(child.descriptionKey)}</p>
                                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      {externalLinks.map((link) => (
                        <a key={link.href} href={link.href} target="_blank" rel="noopener noreferrer"
                          onClick={() => setMobileMenuOpen(false)}
                          className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm border border-transparent transition-all hover:bg-accent/40 text-muted-foreground hover:text-foreground">
                    ...
                    <div className="space-y-3 pt-2 border-t">
                      ...
                      {session?.user ? (
                        <div className="space-y-3">
                          {/* User card */}
                          <div className="flex items-center gap-3 p-3.5 rounded-xl bg-accent/30 border">
                            <div className="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center text-white font-semibold text-base flex-shrink-0">
                              {session.user.name?.charAt(0).toUpperCase() ?? "U"}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-sm font-semibold truncate">{session.user.name}</p>
                              <p className="text-xs text-muted-foreground truncate">{session.user.email}</p>
                              <p className="text-xs text-blue-500 dark:text-blue-400 capitalize mt-0.5">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {[
                              { href: `/${lang}/dashboard`, icon: <LayoutDashboard className="h-4 w-4" />, label: t('navigation.dashboard') },
                              { href: `/${lang}/profile`,   icon: <User className="h-4 w-4" />,            label: t('navigation.profile') },
                              { href: `/${lang}/messages`,  icon: <MessageCircle className="h-4 w-4" />,   label: t('navigation.messages') },
                              { href: `/${lang}/dashboard/settings`, icon: <Globe className="h-4 w-4" />,  label: t('navigation.settings') },
                            ].map(({ href, icon, label }) => (
                              <Button key={href} variant="outline" asChild className="h-auto py-3 flex-col gap-1.5">
                    ...
                    {/* Footer links */}
                    <div className="pt-2 border-t pb-4">
                      <div className="grid grid-cols-2 gap-1 text-xs text-center">
                        {[
                          ...
                          <Link key={href} href={href}
                            onClick={() => setMobileMenuOpen(false)}
                            className="text-muted-foreground hover:text-foreground p-2 rounded-lg hover:bg-accent/30 transition-colors">
                            {label}
                          </Link>
                        ))}
                      </div>
